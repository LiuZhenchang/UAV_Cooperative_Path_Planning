...
Property.selection2=3;                                              % Set path filtering mode 2
...
StartInfo=[ 0*1e3,     50*1e3,      0,      100*1e3;                % positon x, position y, yaw angle, starting arc radius
            0*1e3,     100*1e3,     0,      100*1e3;                % xs, ys, phi_s, R_s
            0*1e3,     150*1e3,     0,      100*1e3;                % unit (mm)
            0*1e3,     200*1e3,     0,      100*1e3];

% Set ending point information
FinishInfo=[1000*1e3,    50*1e3,     0,      100*1e3;               % positon x, position y, yaw angle, starting arc radius
            1000*1e3,    100*1e3,    0,      100*1e3;               % xf, yf, phi_f, R_f
            1000*1e3,    150*1e3,    0,      100*1e3;               % unit (mm)
            1000*1e3,    200*1e3,    0,      100*1e3];

% Set obastacles (threat circle) information
ObsInfo=   [120*1e3,    120*1e3,    50*1e3;                         % positon x, position y, threat circle radius
            150*1e3,    60*1e3,     50*1e3;                         % xo, yo, Ro
            300*1e3,    200*1e3,    40*1e3;                         % unit (mm)
            350*1e3,    50*1e3,     50*1e3;                         
            430*1e3,    0*1e3,      70*1e3;
            420*1e3,    120*1e3,    30*1e3;
            500*1e3,    220*1e3,    50*1e3;
            620*1e3,    150*1e3,    60*1e3;
            700*1e3,    30*1e3,     50*1e3;
            800*1e3,    180*1e3,    60*1e3];
...
Coop_State(1:uav_num)=struct(...                                    % The structure of flight paths information for UAVs
    ...
    'ideal_length',11*1e5,...                                       % Expected path length
...
for uav_index=1:4                                                   % Traverse each UAV
...
Plot_Traj_Coop(Coop_State,ObsInfo,Property,1,2);                    % Plot cooperative path planning results
